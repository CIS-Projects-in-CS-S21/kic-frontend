/**
 * @fileoverview A testing suite for Uploading Media, specifically in terms of uploading images to Google Cloud Storage
 */
...
test('uploadImageIsSuccessful', () => {
    ...
    let your_bytes = Buffer.from(testImageB64, "base64");
    req.setFile(Uint8Array.from(your_bytes));
    req.setFileinfo(file);

    //try to upload file
    client.uploadFile(req,{'Authorization': authString}).then(
        res => {

            console.log(res);
            expect(res.getFileid().toBe(!null));
        })
        .catch(error =>{
            console.log(error);
    })
...
test('uploadImageFailsBecauseBadBase64', () => {
...
test('uploadImageFailsBecauseBadFileInfo', () => {
...
test('uploadImageFailsBecauseBadAuthString', () => {
